...
const CatalogAPI = {
    catalog: [],
    search(input){
        browserHistory.push('/'+input.target.value); 
    },
    getCatalog(){
    ...
    getCatalogSearch(search){
        console.log("s")
        if(!search){
            return this.getCatalog();
        }
        return this.getCatalog().filter((task) => task.itemTitle.toLowerCase().indexOf(search.toLowerCase()) !== -1 );
    },
